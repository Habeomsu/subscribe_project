...
public class CustomLogoutFilter extends GenericFilterBean {
    ...
    private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        ...
        try {
            ...
            ApiResult<?> apiResult = ApiResult.onFailure(ErrorStatus._REFRESH_TOKEN_SIGNATURE_ERROR.getCode(), "유효하지 않은 REFRESH JWT 서명입니다.", null);
            JsonResponseUtil.sendJsonResponse(response, HttpServletResponse.SC_UNAUTHORIZED, apiResult);
            return;
        } catch (JwtException e) {
            // 기타 JWT 관련 오류 처리
            ApiResult<?> apiResult = ApiResult.onFailure(ErrorStatus._INVALID_REFRESH_TOKEN.getCode(), "유효하지 않은 REFRESH JWT입니다.", null);
            JsonResponseUtil.sendJsonResponse(response, HttpServletResponse.SC_UNAUTHORIZED, apiResult);
            return;
        }
